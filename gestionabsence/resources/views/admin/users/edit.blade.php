    ...
    <div class="card-body">
        <form method="POST" action="{{ route('admin.users.update', $user) }}">
            ...
            <div class="row g-3">
               ...
               <div class="col-md-12">
    <label class="form-label fw-semibold">Classe (année académique active)</label>
    <select name="idClasse" class="form-select @error('idClasse') is-invalid @enderror">
        <option value="">-- Sélectionner une classe --</option>
        @foreach($classes as $classe)
        <option value="{{ $classe->idClasse }}"
            {{ old('idClasse', $user->etudiant->classeActuelle()?->idClasse) == $classe->idClasse ? 'selected':'' }}>
            {{ $classe->nom }}
        </option>
        @endforeach
    </select>
    @error('idClasse')<div class="invalid-feedback">{{ $message }}</div>@enderror
    <small class="text-muted">Changer la classe ici met à jour l'inscription de l'année académique en cours.</small>
</div>
                <div class="col-md-12">
